...
function releaseIcons () {
  return src('src/icons/**/*').pipe(dest('cc100/icons'))
}
...
function releaseLocal () {
  return src('src/locales/**/*').pipe(dest('cc100/locales'))
}

function releasePublicData () {
  return src('src/public/**/*').pipe(dest('cc100/public'))
}

function cleanProject () {
  return src(['cc100', 'docs/gen', 'jcoverage'], { allowEmpty: true })
...
function releaseJSContent (cb) {
  ...
  pump([
    src('src/**/*.js')
      ...
      .pipe(uglify())
      .pipe(sourcemaps.write('maps')), dest('cc100')],
  cb)
}
